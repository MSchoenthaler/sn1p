...
import { broadcast, joinChannel, leaveChannel, sendWs } from "./ws-hub.js";
...
export function createDocService() {
  return {
    ...
    async handleWsConnection(ws, req) {
      ...
      const name = sanitizeDocName(url.searchParams.get("doc"));
      const authToken = url.searchParams.get("auth");
      const access = await authorizeExistingDocAccess(name, authToken);
      ...
      const joinIfNeeded = () => {
        if (!ws._doc) joinChannel(name, ws);
      };
